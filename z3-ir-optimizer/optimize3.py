...
def simulate_optimized_cpu(ops, args, init_state, s, target_length):
    ...
    for i in range(target_length):
        ...
        s.add(z3.Or(
            z3.And(op == OP_ADDI, next_acc == (acc + arg) & 0xFF, arg <= 7, arg >= -8),
            z3.And(op == OP_NAND, next_acc == (~acc) & 0xFF, arg == 0)
        ))
...
def optimize(instructions, target_length):
    ...
    original_states = simulate_cpu_array(instructions, acc_init, s, prefix="orig")
...
if __name__ == "__main__":
    instructions = ["addi 1", "addi 1", "addi 1", "nand"]
    print("\nOriginal Instructions:", instructions)
    target_length = len(instructions) - 1
