...
def translate_problem_layout(
    problem: list[ProblemSegmentLabels],
) -> list[ProblemSegmentIndexes]:
    translated_problem = []
            ...
            translated_problem.append(
                ProblemSegmentIndexes(
                    ...
                    jump_index=next(
                        ...
                        if p.segment_id == instr.jump_label
                    ),
                )
            )
...
imms = json.load(open("./wvrn/solutions-16bit.json"))
...
def try_solve_adresses(
    problem: list[ProblemSegmentIndexes], starting_adress, buffer, timeout
) -> list[SolutionSection] | None:
    ...
    if solver.check() == sat:
        ...
        for i in range(num_sectors):
            ...
            if i in label_positions:
                ...
                section = SolutionSection(address=addres, length=length, buffer=buffer)
...
def find_adresses(problem, starting_adress, timeout=2000):
    logging.debug(
        f"Resolving adresses for program with {len(problem)} sections, timeout: {timeout}, starting adress: {starting_adress}"
    )
    ...
    for i in range(2, 5):
        ...
        solution = try_solve_adresses(
            problem, starting_adress, buffer=i, timeout=timeout
        )
        if solution:
            return solution
    else:
...
def get_jump_sequence(target, brc_instruction: Line):
    ...
    if brc_instruction.mached_command[0] not in BRC_COMMANDS:
        raise error.CompilerError(
            None, "Logic error: Jumps solution is in par with problem"
        )
    ...
    for section in solution:
        ...
        if jump_len:
            ...
            if len(jump_seq) != jump_len:
                raise error.CompilerError(
                    None, "Logic error: Jump sequence lenght does not match jump lenght"
                )

            for instr in jump_seq:
                new_program.append(
                    Line(
                        line_index_in_file=jump_instruction.line_index_in_file,
                        comment=jump_instruction.comment if 'comment' in jump_instruction else None,
                        line=instr,
        ...
        for _ in range(buffer):
            new_program.append(
                Line(line_index_in_file=None, line=fill, physical_adress=current_adress)
            )
            current_adress += 1
...
def find_label_physical_adresses(solution, problem, label_segments):
    ...
    for label, segment_id in label_segments.items():
        for i, instr in enumerate(problem):
            ...
            raise error.CompilerError(
                None, f"Logic error: Label {label} not found in solution"
            )
...
def solve_adresses(program, context: Context):
    ...
    if solution:
        ...
        context.physical_adresses = find_label_physical_adresses(
            solution, problem, label_segments
        )
